...
{{-- @php
$isDisabled1 = function($pksData) {
    return isset($pksData) && $bank1-> =='mandiri' && $bank_timer >24  ? 'disabled' : '';
};

$isDisabled2 = function($pksData) {
    return isset($pksData) && $bank2-> =='mandiri' && $bank_timer >48 && $bank_timer <24  ? 'disabled' : '';
};


$isDisabled3 = function($pksData) {
    return isset($pksData) && $bank3-> =='mandiri' && $bank_timer >72 && $bank_timer <48  ? 'disabled' : '';
};


$isExpired1 = function($pksData) {
    if (isset($pksData) && $pksData->bank1 == 'mandiri') {
        if ($pksData->bank_timer > 24) {
            return 'Expired';
        }
         elseif ($pksData->bank_timer < 24) {
            return 'Open';
        }
    }
    return ''; // Default return value when conditions are not met
};

$isExpired2 = function($pksData) {
    if (isset($pksData) && $pksData->bank2 == 'mandiri') {
        if ($pksData->bank_timer > 48) {
            return 'Expired';
        }
         elseif ($pksData->bank_timer > 24 && $pksData->bank_timer < 48 ) {
            return 'Open';
        }
    }
    return ''; // Default return value when conditions are not met
    ...
    <div class="container-fluid">
        <div class="d-flex flex-wrap mb-4 align-items-center search-filter">
            ...
            <div class="mb-0 me-3 mt-3 mt-sm-0 mb-2">
                <form id="filterForm" action="{{ route('home.filterbank') }}" method="GET">
                    <select class="default-select style-1 w-auto" aria-label="Default select example" name="filter" onchange="document.getElementById('filterForm').submit();">
            ...
            <div class="container mt-5">
                ...
                <form id="pks-form" action="{{ route('searchresultbank') }}" method="GET">
                ...
                <div class="table-responsive">
                    <table class="table display mb-4 dataTablesCard table-responsive-xl card-table" id="example5">
                        ...
                        <tbody>
                            ...
                            {
                                if($pks->bank1=='mandiri')
                                    {$keterangan=$pks->bank1_status;
                                    $timer=(24-$pks->bank_timer);
                                    }
                                elseif ($pks->bank2=='mandiri') {
                                    $keterangan=$pks->bank2_status;
                                    $timer=(48-$pks->bank_timer);
                                    $timer_wait=($pks->bank_timer-24);
                                }
                                else {
                                    $keterangan=$pks->bank3_status;
                                    $timer=(72-$pks->bank_timer);
                                    $timer_wait=($pks->bank_timer-48);
                                }
                            }
                            else {
                                $keterangan='Closed';
                            ...
                            @endphp
                                ...
                                <td>
                                    ...
                                    @if($keterangan == 'soon')
                                    <a class="btn rounded btn-outline-warning me-3 ms-auto" href="#">{{ $keterangan }}</a>
                                    @elseif($keterangan == 'open')
                                        <a class="btn rounded btn-outline-success me-3 ms-auto" href="#">{{$keterangan }}</a>
                                    @else
                                        <a class="btn rounded btn-outline-dark me-3 ms-auto" href="#">{{ $keterangan}}</a>
                                    @endif
                                        <div class="dropdown float-end custom-dropdown mb-0  c-pointer">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end">
                                                @php
                                                    $isDisabled = function($keterangan, $nomor_akun_bank) {
                                                        return ($keterangan === 'soon' || $keterangan === 'expired' || $nomor_akun_bank != '0') ? 'disabled' : '';
                                                    };
                                                @endphp

                                                {{-- <a class="dropdown-item" href="{{ route('bank.inputaccountbank', ['id_pks' => $pks->id_pks]) }} {{$isDisabled($keterangan)}}">Details</a> --}}
                                                <button 
                                                    class="dropdown-item btn btn-link" 
                                                    onclick="window.location.href='{{ route('bank.inputaccountbank', ['id_pks' => $pks->id_pks]) }}'" 
                                                    {{ $isDisabled($keterangan,$nomor_akun_bank) }}>
                                                    Details
                                                </button>
                                                <a class="dropdown-item" href="{{ route('monitoring.bank', ['id_pks' => $pks->id_pks]) }}">Monitoring</a>
