...
@section('content')
    ...
    <div class="container-fluid">
        ...
        $isDisabled = function($pks) {
            return isset($pks) && $pks->status != 'baru' ? 'disabled' : '';
        };
    @endphp
        <form action="{{ url('pks/approve/' . $pks->id_pks) }}" method="POST" class="d-flex justify-content-end">
            @csrf
            <button type="submit" class="btn btn-success" {{$isDisabled($pks)}} onclick="return confirm('Apakah Anda Yakin Approve PKS berikut?');">Approve</button>
        </form>
        <br>
        <form id="reject-form" action="{{ url('pks/reject/' . $pks->id_pks) }}" method="POST" class="d-flex justify-content-end">
            @csrf
            <input type="hidden" name="alasan_reject" id="alasan_reject">
            <button type="submit" class="btn btn-danger" {{ $isDisabled($pks) }} onclick="return confirmReject()">Reject</button>
        </form>
...
<script>
      function confirmReject() {
        ...
        document.getElementById('alasan_reject').value = alasan;
        return confirm('Are you sure you want to reject the PKS with the following reason: ' + alasan + '?');
    }
